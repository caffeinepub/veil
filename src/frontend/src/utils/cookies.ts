...
export function setAdminSessionCookie(token: string): void {
  ...
  expires.setDate(expires.getDate() + COOKIE_EXPIRY_DAYS);
  const secure = window.location.protocol === "https:" ? "; Secure" : "";
  document.cookie = `${ADMIN_SESSION_COOKIE_NAME}=${encodeURIComponent(token)}; expires=${expires.toUTCString()}; path=/; SameSite=Strict${secure}`;
}

export function getAdminSessionCookie(): string | null {
  const name = `${ADMIN_SESSION_COOKIE_NAME}=`;
  const decodedCookie = decodeURIComponent(document.cookie);
  const parts = decodedCookie.split(";");
  for (let i = 0; i < parts.length; i++) {
    let c = parts[i].trim();
...
export function clearAdminSessionCookie(): void {
  document.cookie = `${ADMIN_SESSION_COOKIE_NAME}=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; SameSite=Strict`;
}
